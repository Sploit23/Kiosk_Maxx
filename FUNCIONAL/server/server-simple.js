...
function getImagesFolderPath() {
    let basePath = config.image_settings?.base_path;
    if (!basePath) {
        const candidates = [
            path.join(userDataPath, 'imagens'),
            path.join(process.cwd(), 'imagens'),
            path.join(path.dirname(process.execPath), 'imagens')
        ];
        basePath = candidates.find(p => fs.existsSync(p)) || candidates[1];
    }
    ...
    if (fs.existsSync(imagesDir)) {
        ...
        console.log(`❌ AVISO: Pasta do dia ${dataFormatada} (formato: ${getDateFormat()}) não encontrada em: ${basePath}`);
        console.log(`📁 Esperada: ${imagesDir}`);
    }
    // Sempre retornar a pasta do dia (mesmo que não exista) para que a UI saiba quando criar/aguardar
    return imagesDir;
}
...
function runPythonOpenPrint(imagePath, keepSeconds = 5, autoOptions = null) {
    return new Promise((resolve, reject) => {
        try {
            const exeCandidates = [];
            try { exeCandidates.push(path.join(process.cwd(), 'abrir_impressao.exe')); } catch (_) {}
            try { exeCandidates.push(path.join(__dirname, '../../abrir_impressao.exe')); } catch (_) {}
            try { if (process.resourcesPath) exeCandidates.push(path.join(process.resourcesPath, 'bin', 'abrir_impressao.exe')); } catch (_) {}
            const exePath = exeCandidates.find(p => fs.existsSync(p));
            if (exePath) {
                const exeArgs = ['--image', imagePath, '--keep', String(keepSeconds)];
                if (autoOptions && autoOptions.format) {
                    exeArgs.push('--format', String(autoOptions.format));
                    const delayMs = typeof autoOptions.enterDelayMs === 'number' ? autoOptions.enterDelayMs : 100;
                    exeArgs.push('--enter_delay_ms', String(delayMs));
                    const startMs = typeof autoOptions.startDelayMs === 'number' ? autoOptions.startDelayMs : 1500;
                    exeArgs.push('--start_delay_ms', String(startMs));
                }
                if (autoOptions && autoOptions.mode) {
                    exeArgs.push('--mode', String(autoOptions.mode));
                }
                if (autoOptions && autoOptions.printer) {
                    exeArgs.push('--printer', String(autoOptions.printer));
                }
                if (autoOptions && typeof autoOptions.copies === 'number') {
                    exeArgs.push('--copies', String(parseInt(autoOptions.copies, 10)));
                }
                const child = spawn(exePath, exeArgs, { stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
                let out = '';
                let err = '';
                ...
                child.on('close', code => {
                    if (code === 0) {
                        ...
                        reject(new Error(err || String(code)));
                    }
                });
                return;
            }
            const pyCandidates = [
                path.join(process.cwd(), 'abrir_impressao.py'),
                path.join(__dirname, '../../abrir_impressao.py')
            ];
            let script = null;
            ...
            if (!script) {
                reject(new Error('open_py_not_found'));
                return;
            }
            const args = [script, '--image', imagePath, '--keep', String(keepSeconds)];
            if (autoOptions && autoOptions.format) {
                args.push('--format', String(autoOptions.format));
                const delayMs = typeof autoOptions.enterDelayMs === 'number' ? autoOptions.enterDelayMs : 100;
                args.push('--enter_delay_ms', String(delayMs));
                const startMs = typeof autoOptions.startDelayMs === 'number' ? autoOptions.startDelayMs : 1500;
                args.push('--start_delay_ms', String(startMs));
            }
            if (autoOptions && autoOptions.mode) {
                args.push('--mode', String(autoOptions.mode));
            }
            if (autoOptions && autoOptions.printer) {
                args.push('--printer', String(autoOptions.printer));
            }
            if (autoOptions && typeof autoOptions.copies === 'number') {
                args.push('--copies', String(parseInt(autoOptions.copies, 10)));
            }
            ...
            const tryRun = () => {
                ...
                const cmd = runners[idx++];
                const child = spawn(cmd, args, { stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
                let out = '';
                let err = '';
                child.stdout.on('data', d => { out += d.toString(); });
                child.stderr.on('data', d => { err += d.toString(); });
                child.on('error', () => { tryRun(); });
                child.on('close', code => {
                    if (code === 0) {
                        done = true;
                        resolve({ success: true, output: out.trim() });
...
function runPowerShellAutomation(format, enterDelayMs = 100) {
    return new Promise((resolve, reject) => {
        const cfg = loadJSON(getConfigPath('printer_config.json'), {});
        const keyMap = (cfg && cfg.automation_keys) || { '10x15': 'a', '15x20': 'f', 'bolas': 'a' };
        const key = (keyMap[String(format || '').toLowerCase()] || '').trim().toLowerCase();
        const vk = key === 'a' ? 0x41 : key === 'f' ? 0x46 : null;
        const sendKeySegment = vk != null ? `[U32]::keybd_event(${vk}, 0, 0, [UIntPtr]::Zero); Start-Sleep -Milliseconds 50; [U32]::keybd_event(${vk}, 0, 2, [UIntPtr]::Zero)` : '';
        const delay = Math.max(parseInt(enterDelayMs, 10) || 0, 0);
        const sendEnterSegment = `Start-Sleep -Milliseconds ${delay}; [U32]::keybd_event(0x0D, 0, 0, [UIntPtr]::Zero); Start-Sleep -Milliseconds 50; [U32]::keybd_event(0x0D, 0, 2, [UIntPtr]::Zero); try { $ws.SendKeys('{ENTER}') } catch {}`;
        const ps = `
...
while ((Get-Date) -lt $deadline -and $found -eq [IntPtr]::Zero) {
  [U32]::EnumWindows([U32+EnumWindowsProc]{ param([IntPtr]$h)
    if (-not [U32]::IsWindowVisible($h)) { return $true }
    $len = [U32]::GetWindowTextLength($h)
    $sb = New-Object System.Text.StringBuilder($len+1)
    if ($len -gt 0) { [U32]::GetWindowText($h, $sb, $sb.Capacity) | Out-Null }
    $t = $sb.ToString()
    $clsSb = New-Object System.Text.StringBuilder(256)
    [U32]::GetClassName($h, $clsSb, $clsSb.Capacity) | Out-Null
    $cls = $clsSb.ToString()
    if ($t -like '*Imprimir Imagens*' -or $t -like '*Print Pictures*' -or $cls -eq '#32770') { $script:found = $h; return $false }
    return $true
  }, [IntPtr]::Zero) | Out-Null
  Start-Sleep -Milliseconds 100
}
if ($found -ne [IntPtr]::Zero) {
  [U32]::ShowWindow($found, 9) | Out-Null
  [U32]::BringWindowToTop($found) | Out-Null
  [U32]::SetForegroundWindow($found) | Out-Null
  $pid = 0
  [void][U32]::GetWindowThreadProcessId($found, [ref]$pid)
  try { $ws.AppActivate($pid) } catch {}
...
`;
        exec(ps, { shell: 'powershell.exe' }, (err) => {
            if (err) {
                reject(new Error(err.message || 'fail'));
                return;
            }
...
function runPythonAutomation(format, enterDelayMs = 100) {
    return runPowerShellAutomation(format, enterDelayMs);
}
...
function openPrintDialogAndAutomate(fullImagePath, format, targetPrinter) {
    return new Promise((resolve, reject) => {
        const setDefault = targetPrinter ? `$net = New-Object -ComObject WScript.Network; try { $net.SetDefaultPrinter(\"${targetPrinter}\") } catch {}` : '';
        const ps = `
$ErrorActionPreference = 'SilentlyContinue'
${setDefault}
...
$shell.ShellExecute("${fullImagePath}", '', '', 'print', 1)
...
while ((Get-Date) -lt $deadline -and $found -eq [IntPtr]::Zero) {
  [U32]::EnumWindows([U32+EnumWindowsProc]{ param([IntPtr]$h)
    ...
    $cls = $clsSb.ToString()
    $pid = 0
    [void][U32]::GetWindowThreadProcessId($h, [ref]$pid)
    $p = $null
    try { $p = Get-Process -Id $pid -ErrorAction SilentlyContinue } catch {}
    ...
    if (
      $cls -eq '#32770' -and (
        $name -match 'rundll32|dllhost|explorer|ApplicationFrameHost'
      )
    ) { $script:found = $h; return $false }
    if ($t -like '*Imprimir Imagens*' -or $t -like '*Print Pictures*') { $script:found = $h; return $false }
    return $true
  }, [IntPtr]::Zero) | Out-Null
  Start-Sleep -Milliseconds 200
}
if ($found -ne [IntPtr]::Zero) { [U32]::SetForegroundWindow($found) | Out-Null; Start-Sleep -Milliseconds 300; $pid = 0; [void][U32]::GetWindowThreadProcessId($found, [ref]$pid); try { $ws.AppActivate($pid) } catch {} }
`;
...
const server = http.createServer(async (req, res) => {
    ...
    function resolveImagePath(requestedPath) {
        const imagesDir = getImagesFolderPath();
        const basePath = config.image_settings?.base_path || imagesDir;
        const allowedExtensions = (config.image_settings?.allowed_extensions) || ['.jpg', '.jpeg', '.png', '.gif'];
        let fullImagePath = requestedPath;
        if (!path.isAbsolute(fullImagePath)) {
            fullImagePath = path.join(imagesDir, requestedPath);
        }
        if (!fs.existsSync(fullImagePath) && !path.extname(requestedPath)) {
            for (const ext of allowedExtensions) {
                const candidate = path.join(imagesDir, requestedPath + ext);
        ...
        if (!fs.existsSync(fullImagePath)) {
            const direct = path.join(basePath, requestedPath);
        ...
        if (!fs.existsSync(fullImagePath)) {
            try {
                ...
                if (searchRoot) {
                    const searchCmd = `powershell -NoProfile -Command "Get-ChildItem -Path \"${searchRoot}\" -Recurse -File | Where-Object { $_.Name -eq \"${path.basename(requestedPath)}\" } | Select-Object -First 1 -ExpandProperty FullName"`;
    ...
    if (pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/images' && method === 'GET') {
            ...
            if (!imagesDir || !fs.existsSync(imagesDir)) {
                res.writeHead(404, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({
                    erro: `Pasta do dia não encontrada`,
                    status: 'daily_folder_not_found',
                    expected_path: imagesDir,
                    expected_folder: path.basename(imagesDir || ''),
                    current_format: currentFormat,
                    pasta_base: basePath,
                    solucao: `Crie a pasta do dia no caminho acima e adicione fotos.`
                }));
                return;
            }
            ...
            try {
                ...
                if (arquivosImagem.length === 0) {
                    res.writeHead(404, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({
                        erro: `Nenhuma imagem encontrada na pasta do dia`,
                        status: 'no_images_found',
                        folder_path: imagesDir,
                        current_format: currentFormat,
                        pasta_base: basePath,
                        solucao: 'Adicione fotos na pasta do dia.'
                    }));
                    return;
                }
        ...
        if (normalizedPath === '/api/print-dialog-manual' && method === 'POST') {
            let body = '';
            ...
            req.on('end', async () => {
                try {
                    ...
                    if (!imagePath) {
                        ...
                        res.end(JSON.stringify({ success: false, error: 'imagePath_required' }));
                        return;
                    }
                    const fullImagePath = resolveImagePath(imagePath);
                    if (!fs.existsSync(fullImagePath)) {
                        res.writeHead(404, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: false, error: 'image_not_found', path: fullImagePath }));
                        return;
                    }
                    try {
                        const t0 = Date.now();
                        console.log(`[LOG] manual_print_start file=${fullImagePath} at ${new Date().toISOString()}`);
                        const cfg = loadJSON(getConfigPath('printer_config.json'), {});
                        const keepSeconds = typeof payload.keepSeconds === 'number' ? payload.keepSeconds : (cfg.manual_dialog_keep_seconds ?? 300);
                        runPythonOpenPrint(fullImagePath, keepSeconds, null).catch(err => {
                            console.log(`[LOG] manual_print_py_error err=${String(err && err.message || err)}`);
                        });
                        const dt = Date.now() - t0;
                        console.log(`[LOG] manual_print_opened duration=${dt}ms keep=${keepSeconds}s at ${new Date().toISOString()}`);
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: true, method: 'python_open', scheduled: false, keep: keepSeconds }));
                    } catch (e) {
                        res.writeHead(500, { 'Content-Type': 'application/json' });
        ...
        if (pathname === '/api/print-automation' && method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString(); });
            req.on('end', async () => {
                try {
                    ...
                    const enterDelayMs = typeof payload.enterDelayMs === 'number' ? payload.enterDelayMs : 5;
                    if (!format) {
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: false, error: 'format_required' }));
                        return;
                    }
                    try {
                        const result = await runPythonAutomation(format, enterDelayMs);
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: true, method: 'python_automation', details: result }));
                    } catch (e) {
                        res.writeHead(500, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: false, error: String(e.message || e) }));
                    }
        ...
        if (pathname === '/api/personalization' && method === 'GET') {
            try {
                ...
                const defaults = { header_text: 'Fotos Mágicas', header_bg_color: '#2c3e50', header_text_color: '#ffffff', accent_color: '#3498db', list_text_color: '#212529', list_border_radius: 15 };
                const data = loadJSON(pPath, defaults);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify(data));
            } catch (error) {
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ header_text: 'Fotos Mágicas', header_bg_color: '#2c3e50', header_text_color: '#ffffff', accent_color: '#3498db', list_text_color: '#212529', list_border_radius: 15 }));
            }
            return;
        }
            ...
            req.on('end', () => {
                try {
                    ...
                    const payload = {
                        ...
                        list_text_color: String(data.list_text_color || '#212529'),
                        list_border_radius: parseInt(data.list_border_radius || 15, 10),
        ...
        if (normalizedPath === '/api/ui/sidebar-thumbnail-size' && method === 'GET') {
            const imageConfigPath = getConfigPath('image_settings.json');
            const cfg = loadJSON(imageConfigPath, {});
            const size = parseInt(cfg.sidebar_thumbnail_size || 120, 10);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ size }));
            return;
        }

        if (normalizedPath === '/api/ui/sidebar-thumbnail-size' && method === 'POST') {
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', () => {
                try {
                    const data = JSON.parse(body || '{}');
                    const size = parseInt(data.size, 10);
                    if (isNaN(size) || size < 60 || size > 400) {
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: false, error: 'Invalid size' }));
                        return;
                    }
                    const imageConfigPath = getConfigPath('image_settings.json');
                    const cfg = loadJSON(imageConfigPath, {});
                    cfg.sidebar_thumbnail_size = size;
                    saveJSON(imageConfigPath, cfg);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: true, size }));
                } catch (e) {
        ...
        if (normalizedPath === '/api/ui/sidebar-opacity' && method === 'GET') {
            const imageConfigPath = getConfigPath('image_settings.json');
            const cfg = loadJSON(imageConfigPath, {});
            let opacity = parseFloat(cfg.sidebar_bg_opacity ?? 0.10);
            if (isNaN(opacity)) opacity = 0.10;
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ opacity }));
            return;
        }

        if (normalizedPath === '/api/ui/sidebar-opacity' && method === 'POST') {
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', () => {
                try {
                    const data = JSON.parse(body || '{}');
                    let opacity = parseFloat(data.opacity);
                    if (isNaN(opacity) || opacity < 0 || opacity > 1) {
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: false, error: 'Invalid opacity' }));
                        return;
                    }
                    const imageConfigPath = getConfigPath('image_settings.json');
                    const cfg = loadJSON(imageConfigPath, {});
                    cfg.sidebar_bg_opacity = opacity;
                    saveJSON(imageConfigPath, cfg);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        if (normalizedPath === '/api/printer/config-simple' && method === 'GET') {
            const p = loadJSON(getConfigPath('printer_config.json'), {});
            const out = {
                default_format: String(p.user_preferences?.default_format || ''),
                default_printer: String(p.user_preferences?.default_printer || ''),
                automation_start_delay_ms: typeof p.automation_start_delay_ms === 'number' ? p.automation_start_delay_ms : 1500,
                automation_enter_delay_ms: typeof p.automation_enter_delay_ms === 'number' ? p.automation_enter_delay_ms : 500,
                automation_keys: p.automation_keys || { '10x15': 'a', '15x20': 'f', 'bolas': 'a' }
            };
            res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        if (normalizedPath === '/api/printer/config-simple' && method === 'POST') {
            let body = '';
            ...
            req.on('end', () => {
                try {
                    const data = JSON.parse(body || '{}');
                    const cfg = loadJSON(getConfigPath('printer_config.json'), {});
                    cfg.user_preferences = cfg.user_preferences || {};
                    if (typeof data.default_format === 'string') cfg.user_preferences.default_format = data.default_format;
                    if (typeof data.default_printer === 'string') cfg.user_preferences.default_printer = data.default_printer;
                    if (typeof data.automation_start_delay_ms === 'number') cfg.automation_start_delay_ms = data.automation_start_delay_ms;
                    if (typeof data.automation_enter_delay_ms === 'number') cfg.automation_enter_delay_ms = data.automation_enter_delay_ms;
                    if (data.automation_keys && typeof data.automation_keys === 'object') cfg.automation_keys = data.automation_keys;
                    saveJSON(getConfigPath('printer_config.json'), cfg);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        if (normalizedPath === '/api/print' && method === 'POST') {
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
                try {
                    ...
                    try {
                        ...
                        runPythonOpenPrint(fullImagePath, 0, { mode: 'direct', printer: targetPrinter, copies: copies || 1 }).catch(() => {});
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({
                            success: true,
        ...
        if (normalizedPath === '/api/print-dialog' && method === 'POST') {
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
                try {
                    const data = JSON.parse(body || '{}');
                    const { imagePath, format, copies } = data;
                    if (!imagePath) {
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                    ...
                    const fullImagePath = resolveImagePath(imagePath);
                    if (!fs.existsSync(fullImagePath)) {
                        const directRoot = (config.image_settings && config.image_settings.base_path) ? config.image_settings.base_path : '';
                        if (directRoot) {
                            const direct = path.join(directRoot, imagePath);
                            if (fs.existsSync(direct)) fullImagePath = direct;
                        }
                    }
                    ...
                    if (!fs.existsSync(fullImagePath)) {
                        try {
                            const { execSync } = require('child_process');
                            const searchRoot = (config.image_settings && config.image_settings.base_path) ? config.image_settings.base_path : '';
                            if (searchRoot) {
                                const searchCmd = `powershell -NoProfile -Command "Get-ChildItem -Path \"${searchRoot}\" -Recurse -File | Where-Object { $_.Name -eq \"${path.basename(imagePath)}\" } | Select-Object -First 1 -ExpandProperty FullName"`;
                                const found = String(execSync(searchCmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString()).trim();
                                if (found) fullImagePath = found;
                            }
                        } catch (_) {}
                    ...
                    try {
                        const cfg = loadJSON(getConfigPath('printer_config.json'), {});
                        const fmtAuto = String((format || cfg.user_preferences?.default_format || '')).trim();
                        const delayAuto = typeof data.enterDelayMs === 'number' ? data.enterDelayMs : (cfg.automation_enter_delay_ms ?? 100);
                        const startDelayMs = typeof data.autoStartDelayMs === 'number' ? data.autoStartDelayMs : (cfg.automation_start_delay_ms ?? 0);
                        runPythonOpenPrint(fullImagePath, 5, fmtAuto ? { format: fmtAuto, enterDelayMs: delayAuto, startDelayMs } : null).catch(err => {
                            console.log(`[LOG] print_dialog_py_error err=${String(err && err.message || err)}`);
                        });
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: true, method: fmtAuto ? 'python_script' : 'python_open', scheduled: !!fmtAuto }));
                    } catch (e) {
                        res.writeHead(500, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: false, error: String(e.message || e) }));
                    }
