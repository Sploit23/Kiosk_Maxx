...
(function() {
    ...
    async function printCurrentPhoto() {
        ...
        try {
            ...
            const url = new URL(currentSelectedImage, window.location.origin);
            const imagePath = decodeURIComponent(url.pathname.split('/').pop());
            const response = await fetch('/api/print-dialog', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ imagePath, format: selectedPaperSize, copies })
            });
            const result = await response.json();
            if (!response.ok || !result.success) throw new Error(result.error || result.message || 'Falha ao imprimir');
     ...
     async function printWithConfiguredPrinter(file, printerName, paperSize, copies) {
         try {
             ...
             const response = await fetch('/api/print', {
                 method: 'POST',
                 headers: { 'Content-Type': 'application/json' },
                 body: JSON.stringify({ imagePath, printerName, paperSize, copies })
             });
             const result = await response.json();
             if (result.success) {
                 showPrintNotification('Impressão enviada', 'success');
             } else {
                 throw new Error(result.error || result.message || 'Erro na impressão');
             }
         } catch (error) {
