...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadCurrentSettings();
    setupEventListeners();
    loadPrinterSimpleConfig();
    setupPrinterSimpleEvents();
});
...
async function loadSidebarThumbnailConfig() {
    try {
        let r = await fetch('/api/ui/sidebar-thumbnail-size');
        if (!r.ok) r = await fetch('/api/ui/sidebar-thumbnail-size/');
        if (!r.ok) {
            const origin = window.location.origin || 'http://localhost:5000';
            r = await fetch(origin + '/api/ui/sidebar-thumbnail-size');
        }
        if (r.ok) {
            const d = await r.json();
            const s = parseInt(d.size || 120, 10);
            const input = document.getElementById('sidebar-thumb-size-input');
            const label = document.getElementById('sidebar-thumb-size-label');
            if (input) input.value = s;
            if (label) label.textContent = `${s} px`;
            document.documentElement.style.setProperty('--sidebar-thumb-height', `${s}px`);
            try { localStorage.setItem('sidebar_thumb_size', String(s)); } catch (_) {}
        } else {
            let s = 120;
...
async function loadSidebarOpacityConfig() {
    try {
        let r = await fetch('/api/ui/sidebar-opacity');
        if (!r.ok) r = await fetch('/api/ui/sidebar-opacity/');
        if (!r.ok) {
            const origin = window.location.origin || 'http://localhost:5000';
            r = await fetch(origin + '/api/ui/sidebar-opacity');
        }
        if (r.ok) {
            const d = await r.json();
            const v = parseFloat(d.opacity ?? 0.10);
            const input = document.getElementById('sidebar-opacity-input');
            const label = document.getElementById('sidebar-opacity-label');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadSidebarThumbnailConfig();
    loadSidebarOpacityConfig();
...
async function loadPrinterSimpleConfig() {
    try {
        let r = await fetch('/api/printer/config-simple');
        if (!r.ok) r = await fetch('/api/printer/config-simple/');
        if (!r.ok) {
            const origin = window.location.origin || 'http://localhost:5000';
            r = await fetch(origin + '/api/printer/config-simple');
        }
        if (r.ok) {
            const d = await r.json();
            const df = document.getElementById('printer-default-format');
            const dn = document.getElementById('printer-default-name');
            const sd = document.getElementById('printer-start-delay');
            const ed = document.getElementById('printer-enter-delay');
            const k1 = document.getElementById('key-10x15');
            const k2 = document.getElementById('key-15x20');
            const kb = document.getElementById('key-bolas');
            if (df) df.value = d.default_format || '';
            if (dn) dn.value = d.default_printer || '';
            if (sd) sd.value = parseInt(d.automation_start_delay_ms ?? 1500, 10);
            if (ed) ed.value = parseInt(d.automation_enter_delay_ms ?? 500, 10);
            if (k1) k1.value = (d.automation_keys?.['10x15'] || 'a');
            if (k2) k2.value = (d.automation_keys?.['15x20'] || 'f');
            if (kb) kb.value = (d.automation_keys?.['bolas'] || 'a');
        }
    } catch (_) {}
}

function setupPrinterSimpleEvents() {
    const btn = document.getElementById('save-printer-config');
    if (!btn) return;
    btn.addEventListener('click', async () => {
        const df = document.getElementById('printer-default-format')?.value || '';
        const dn = document.getElementById('printer-default-name')?.value || '';
        const sd = parseInt(document.getElementById('printer-start-delay')?.value || '1500', 10);
        const ed = parseInt(document.getElementById('printer-enter-delay')?.value || '500', 10);
        const k1 = (document.getElementById('key-10x15')?.value || 'a').toLowerCase();
        const k2 = (document.getElementById('key-15x20')?.value || 'f').toLowerCase();
        const kb = (document.getElementById('key-bolas')?.value || 'a').toLowerCase();
        const payload = {
            default_format: df,
            default_printer: dn,
            automation_start_delay_ms: sd,
            automation_enter_delay_ms: ed,
            automation_keys: { '10x15': k1, '15x20': k2, 'bolas': kb }
        };
        const req = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) };
        let ok = false;
        try {
            let resp = await fetch('/api/printer/config-simple', req);
            if (!resp.ok) resp = await fetch('/api/printer/config-simple/', req);
            if (!resp.ok) {
                const origin = window.location.origin || 'http://localhost:5000';
                resp = await fetch(origin + '/api/printer/config-simple', req);
            }
            if (resp.ok) ok = true;
        } catch (_) {}
        if (ok) {
            showSuccess('Configuração de impressão salva');
        } else {
            showError('Falha ao salvar configuração');
        }
    });
...
async function loadPersonalization() {
    try {
        ...
        const lt = document.getElementById('personal-list-text-color');
        const br = document.getElementById('personal-list-border-radius');
        ...
        if (lt) lt.value = personalization.list_text_color || '#212529';
        if (br) br.value = String(parseInt(personalization.list_border_radius || 15, 10));
        ...
        if (preview) {
            ...
            preview.style.borderRadius = `${parseInt(personalization.list_border_radius || 15, 10)}px`;
        }
    } catch {}
}
...
function setupPersonalizationEvents() {
    ...
    if (btn) {
        btn.addEventListener('click', async () => {
            const payload = {
                ...
                list_text_color: document.getElementById('personal-list-text-color')?.value || '#212529',
                list_border_radius: parseInt(document.getElementById('personal-list-border-radius')?.value || '15', 10)
            };
            try {
                ...
                if (resp.ok) {
                    ...
                    if (preview) {
                        ...
                        preview.style.borderRadius = `${parseInt(payload.list_border_radius || 15, 10)}px`;
                    }
                } else {
    ...
    const sidebarThumbInput = document.getElementById('sidebar-thumb-size-input');
    const sidebarThumbLabel = document.getElementById('sidebar-thumb-size-label');
    const saveSidebarThumb = document.getElementById('save-sidebar-thumb-size');
    if (sidebarThumbInput && sidebarThumbLabel) {
        sidebarThumbInput.addEventListener('input', () => {
            const v = parseInt(sidebarThumbInput.value, 10);
            sidebarThumbLabel.textContent = `${v} px`;
            document.documentElement.style.setProperty('--sidebar-thumb-height', `${v}px`);
        });
    }
    if (saveSidebarThumb && sidebarThumbInput) {
        saveSidebarThumb.addEventListener('click', async () => {
            const v = parseInt(sidebarThumbInput.value, 10);
            const reqOpts = {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            let ok = false;
            try {
                let resp = await fetch('/api/ui/sidebar-thumbnail-size', reqOpts);
                if (!resp.ok) {
                    resp = await fetch('/api/ui/sidebar-thumbnail-size/', reqOpts);
                }
                if (!resp.ok) {
                    const origin = window.location.origin || 'http://localhost:5000';
                    resp = await fetch(origin + '/api/ui/sidebar-thumbnail-size', reqOpts);
                }
                if (resp.ok) {
                    ok = true;
                    showSuccess(`Tamanho da lista salvo: ${v}px`);
                }
            } catch (e) {}
            if (!ok) {
                try {
                    localStorage.setItem('sidebar_thumb_size', String(v));
                    document.documentElement.style.setProperty('--sidebar-thumb-height', `${v}px`);
                    showSuccess(`Tamanho da lista salvo localmente: ${v}px`);
                } catch (_) {
                    showError('Erro ao salvar tamanho da lista');
                }
            }
        });
    }

    const sidebarOpacityInput = document.getElementById('sidebar-opacity-input');
    const sidebarOpacityLabel = document.getElementById('sidebar-opacity-label');
    const saveSidebarOpacity = document.getElementById('save-sidebar-opacity');
    if (sidebarOpacityInput && sidebarOpacityLabel) {
        sidebarOpacityInput.addEventListener('input', () => {
            const v = parseFloat(sidebarOpacityInput.value);
            sidebarOpacityLabel.textContent = `${Math.round(v * 100)}%`;
            document.documentElement.style.setProperty('--sidebar-bg-opacity', String(v));
        });
    }
    if (saveSidebarOpacity && sidebarOpacityInput) {
        saveSidebarOpacity.addEventListener('click', async () => {
            const v = parseFloat(sidebarOpacityInput.value);
            const reqOpts = {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            let ok = false;
            try {
                let resp = await fetch('/api/ui/sidebar-opacity', reqOpts);
                if (!resp.ok) {
                    resp = await fetch('/api/ui/sidebar-opacity/', reqOpts);
                }
                if (!resp.ok) {
                    const origin = window.location.origin || 'http://localhost:5000';
                    resp = await fetch(origin + '/api/ui/sidebar-opacity', reqOpts);
                }
                if (resp.ok) {
                    ok = true;
                    showSuccess(`Opacidade da lista salva: ${Math.round(v * 100)}%`);
                }
            } catch (e) {}
            if (!ok) {
                try {
                    localStorage.setItem('sidebar_bg_opacity', String(v));
                    document.documentElement.style.setProperty('--sidebar-bg-opacity', String(v));
                    showSuccess('Opacidade da lista salva localmente');
                } catch (_) {
                    showError('Erro ao salvar opacidade da lista');
                }
            }
        });
    }