...
class PrintCart {
    ...
    // Função para controlar visibilidade do botão do carrinho
    setCartButtonVisibility(visible) {
        const cartButton = document.getElementById('cart-button');
        if (cartButton) {
            cartButton.style.display = visible ? 'flex' : 'none';
        }
    }

    // Função para verificar se há fotos disponíveis
    checkPhotosAvailability() {
        // Verifica se há fotos carregadas no sistema
        const hasPhotos = window.grupos && Object.keys(window.grupos).length > 0;
        this.setCartButtonVisibility(hasPhotos);
        return hasPhotos;
    }
